...
class MinimaxPlayer(IsolationPlayer):
    ...
    def minimax(self, game, depth):
        ...
        **********************************************************************
        ...
        for m in legal_moves:
            v,_ = self.min_value(game.forecast_move(m),depth-1)
            if v >= best_score:
                best_score = v
                best_move = m
        return best_move
    ...
    def min_value(self,game,depth):
        ...
        for m in legal_moves:
            #print("----------Moves in Min---------------")
            #print(m)
            score,move = self.max_value(game.forecast_move(m),depth-1)
            if score <= best_score:
               best_score = score
               best_move = move
    ...
    def max_value(self,game,depth):
        ...
        for m in legal_moves:
            score,move = self.min_value(game.forecast_move(m), depth-1)
            if score >= best_score:
                best_score = score
                best_move = move
        return best_score,best_move
...
class AlphaBetaPlayer(IsolationPlayer):
    ...
    def alphabeta(self, game, depth, alpha=float("-inf"), beta=float("inf")):
        ...
        **********************************************************************
        ...
        if depth == 0:
            return self.score(game,self), (-1,-1)
        if len(legal_moves) != 0:
            best_move_ret=legal_moves[0]
        '''else:
            best_move=(-1,-1)'''

        best_score = float("-inf")
        for m in legal_moves:
            v,_ = self.min_value_alphaBeta(game.forecast_move(m),depth-1,alpha,beta)
            if v > best_score:
                best_score = v
                best_move = m
            '''if best_score >= beta:
    ...
    def terminal_test_alphaBeta(self,game):
        """ Return True if the game is over for the active player
        and False otherwise.
        """
        #if self.time_left() < self.TIMER_THRESHOLD:
    ...
    def max_value_alphaBeta(self,game,depth,alpha,beta):
        ...
        best_score = float("-inf")
        for m in legal_moves:
             score,move= self.min_value_alphaBeta(game.forecast_move(m),depth-1,alpha,beta)
             if score > best_score:
                best_score = score
                best_move = move
             if best_score >= beta:
    ...
    def min_value_alphaBeta(self,game,depth,alpha,beta):
        ...
        #best_move=None
        for m in legal_moves:
            score,move = self.max_value_alphaBeta(game.forecast_move(m),depth-1,alpha,beta)
            if score < best_score:
               best_score = score
               best_move = move
            if best_score <= alpha:
